...
function isCompleteMint(mintId: string): boolean {
  const event = MintEvent.load(mintId)
...
export function handleTransfer(event: Transfer): void {
  ...
  if (factory && pair && from.toHexString() == ADDRESS_ZERO) {
    // update total supply
    ...
    if (token0 && token1) {
      ...
      if (derivedETH0 && derivedETH1) {
        ...
        if (bundle) {
          ...
          // update the LP position
          const burner = burn.sender
          if (burner) {
            let liquidityPosition = createLiquidityPosition(event.address, Address.fromBytes(burner))
            createLiquiditySnapshot(liquidityPosition, event)
          }
